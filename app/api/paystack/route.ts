...
// There's no Sentry/monitoring service wired in — without this, a payment
// that succeeded on Paystack's side but failed to actually upgrade the user
// (or any other uncaught failure here) would only be visible by manually
// checking Vercel logs. That's the one failure mode in this whole app where
// silence is worst: money changed hands but the product didn't deliver.
async function notifyAdmin(context: string, err: unknown) {
  try {
    const detail = err instanceof Error ? err.message : String(err);
    await sendMessage(ADMIN_TELEGRAM_ID, `⚠️ Ava payment error in ${context}:\n${detail.slice(0, 500)}`);
  } catch {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!telegramId) {
      ...
      await notifyAdmin('missing telegram_id', `reference=${reference}`);
      return NextResponse.json({ received: true });
    }
    ...
    // further out than intended), insert a duplicate payment record, and
    // send a second "You're now Premium" message.
    ...
    if (!valid) {
      ...
      await notifyAdmin('verification failed', `reference=${reference}, telegramId=${telegramId}`);
      return NextResponse.json({ received: true });
    }
    ...
    if (error || !user) {
      ...
      // Money was already verified as captured by Paystack at this point —
      // this is the "paid but didn't get what they paid for" case, worth
      // waking up for rather than discovering days later from a complaint.
      await notifyAdmin('paid but upgrade failed', `telegramId=${telegramId}, reference=${reference}, dbError=${error?.message}`);
      return NextResponse.json({ received: true });
    }
    ...
    console.error('Paystack webhook uncaught error:', err);
    await notifyAdmin('uncaught exception', err);
    return NextResponse.json({ received: true });
  }
}
